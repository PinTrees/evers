...
class DialogRE extends StatelessWidget {

  static dynamic showCreateRe(BuildContext context, { String? ctUid }) async {
    var dividHeight = 6.0;
                ...
                var w = Container(
                  ...
                  child: Row(
                      children: [
                        ...
                        WidgetT.excelInput(context, '$i::df거래처', width: 250, index: i,
                          onEdite: (i, data) async {
                            if(revContract[i].csName == data) return;
                            revContract[i].csName = data;
                            ...
                            if(selectContract != null) {
                              revContract[i] = selectContract;
                              var cs = await SystemT.getCS(selectContract.csUid);
                              revContract[i].csName = cs.businessName;
                            }
                          },
                          text: '${revContract[i].csName} / ${revContract[i].ctName}', value: '',
                        ),
                        ...
                        Expanded(
                          child: WidgetT.excelInput(context, '$i::re.item', width: 200, index:i,
                            onEdite: (i, data) async {
                              if(data == '') return;
                        ...
                        WidgetT.excelInput(context, '$i::re.count', width: 80, index: i,
                          onEdite: (i, data) {
                        ...
                        InkWell(
                            ...
                            child: WidgetT.iconMini(Icons.cancel, size: 32),
                        ),
                      ]
                  ),
                );
                        ...
                        Row(
                          children: [
                            InkWell(
                                onTap: () {
                                  revenueList.add(Revenue.fromDatabase({ 'revenueAt': DateTime.now().microsecondsSinceEpoch, }));
                                  if(ctUid != null) revContract.add(ct);
                                  else revContract.add(Contract.fromDatabase({}));
                                  FunT.setStateDT();
                                },
                                child: Container(
                      ...
                      Expanded(
                        child:TextButton(
                            onPressed: () async {
                              ...
                              if(revenueList.length < 1) { WidgetT.showSnackBar(context, text: '매출을 추가해 주세요.'); return; }
