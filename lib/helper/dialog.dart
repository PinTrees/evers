...
class DialogT extends StatelessWidget {
  ...
  /// 계약화면에서 매출을 추가할 경우 - 계약 변경 불가,
  static dynamic showCreateReWithCt(BuildContext context, Contract ct) async {
    var dividHeight = 6.0;
    var payment = '매출';
    var payType = '';

    await ct.update();
    Map<String, Revenue> revenueMaps = {};
          ...
          return  StatefulBuilder(
            builder: (BuildContext context, StateSetter setStateS) {
              ...
              for(int i = 0; i < ct.contractList.length; i++) {
                var cl = ct.contractList[i] as Map;
                var item = SystemT.getItem(cl['item'] ?? '') ?? Item.fromDatabase({});
                      ...
                      child: Container( height: 28,
                        child: Row(
                            children: [
                              ...
                              WidgetT.excelGrid( width: 200, label: '품목', text: item.name,),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid(label: '단위', text:item.unit, width: 50),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid(width: 100, label: '수량', text: StyleT.krw(cl['count']),),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid(width: 100, label: '잔량', text: StyleT.krw(ct.getItemCountRevenue(cl['id']).toString()),),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelInput(context, '$i::매출수량', width: 150, index: i, label: '매출수량',
                                onEdite: (i, data) {
                                  var a = int.tryParse(data) ?? 0;
                                  var b = ct.getItemCountRevenue(cl['id']);
                                  if(a > b) {
                                    WidgetT.showSnackBar(context, text: '허용량을 초과했습니다. 다시 입력해 주세요');
                                    return;
                                  } else if (a > 0){
                                    var re = Revenue.fromDatabase({});
                                    re.ctIndexId = cl['id'];
                                    re.revenueAt = DateTime.now().microsecondsSinceEpoch;
                                    re.unitPrice = int.tryParse(cl['unitPrice']) ?? 0;
                                    re.item = cl['item'];
                                    re.count = a;
                                    revenueMaps['$i'] = re;
                                  } else {
                                    revenueMaps.remove('$i');
                                  }
                                },
                                text: StyleT.krw((revenueMaps['$i'] == null) ? '' : revenueMaps['$i']!.count.toString()),
                                value: (revenueMaps['$i'] == null) ? '' : revenueMaps['$i']!.count.toString(),
                              ),
                              WidgetT.dividViertical(height: 28),
                              TextButton(
                                  onPressed: () {
                                    WidgetT.showSnackBar(context, text: '버튼 선택방식으로 변경 예정');
                                  },
                                  style: StyleT.buttonStyleNone(round: 0, elevation: 0, padding: 0, color: Colors.transparent, strock: 1),
                                  child: Container( height: 28, width: 28,
                                    child: WidgetT.iconMini(Icons.add),)
                              ),
                            ]
                        ),
                      )),
              ...
              for(int i = 0; i < revenueMaps.length; i++) {
                var re = revenueMaps.values.elementAt(i);
                var item = SystemT.getItem(re.item) ?? Item.fromDatabase({});
                var unitPrice = re.unitPrice = item.unitPrice; //int.tryParse((ctl['unitPrice'] != '') ? ctl['unitPrice'] : item?.unitPrice.toString()) ?? 0;

                if(ct.vatType == 0) { re.totalPrice = unitPrice * re.count;  re.vat = (re.totalPrice / 11).round(); re.supplyPrice = re.totalPrice - re.vat; }
                if(ct.vatType == 1) { re.vat = ((unitPrice * re.count) / 10).round(); re.totalPrice = unitPrice * re.count + re.vat;  re.supplyPrice = unitPrice * re.count; }
                      ...
                      child: Container( height: 28,
                        child: Row(
                            children: [
                              ...
                              WidgetT.excelGrid(label: '단위', text:item.unit, width: 50),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid(width: 100, label: '수량', text: StyleT.krw(re.count.toString()),),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid(width: 100, label: '단가', text: StyleT.krw(re.unitPrice.toString()),),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid(width: 150, label: '공급가액', text: StyleT.krw(re.getSupplyPrice().toString()),),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid( width: 100, label: 'VAT', text: StyleT.krw(re.vat.toString()), ),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelGrid(width: 150,label: '합계', text: StyleT.krw(re.getTotalPrice().toString()),),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelInput(context, '$i::매출일자', width: 150, label: '매출일자',
                                onEdite: (i, data) { re.revenueAt = StyleT.dateEpoch(data); },
                                text: StyleT.dateInputFormatAtEpoch(re.revenueAt.toString()),
                              ),
                              WidgetT.dividViertical(height: 28),
                              WidgetT.excelInput(context, '$i::메모', width: 200, index: i, label: '메모',
              ...
              return AlertDialog(
                ...
                title: Column(
                  children: [
                    Container(padding: EdgeInsets.all(8), child: Text('계약추가', style: StyleT.titleStyle(bold: true))),
                  ...
                  child: Container(
                    ...
                    child: Column( crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        Row(
                          children: [
                            WidgetT.title('계약상세정보', width: 100 ),
                            Container(
                              ...
                              child: TextButton(
                                  ...
                                  child: Container(padding: EdgeInsets.all(0), child: Row(
                                    ...
                                    children: [
                                      Container( height: 28, width: 28,
                                        child: WidgetT.iconMini(Icons.refresh),),
                                      WidgetT.title('새로고침'),
                                      SizedBox(width: 6,),
                                    ],
                                  ))
                        ...
                        SizedBox(height: dividHeight,),
                        for(var w in widgetW) w,

                        SizedBox(height: 18,),
                        Row(
                          children: [
                            WidgetT.title('매출목록', width: 100 ),
                            /*Container(
                              padding: EdgeInsets.only(right: dividHeight),
                              child: TextButton(
                                  onPressed: () {
                        ...
                        Row(
                          children: [
                            ...
                            Container( height: 28,
                              child: TextButton(
                                  onPressed: () async {
                                    if(revenueMaps.length == 0) {
                                      WidgetT.showSnackBar(context, text: '매출값이 입력되지 않았습니다.');
                                      return;
                                    }
                                    if(payment == '즉시' && payType == '') {
                                      WidgetT.showSnackBar(context, text: '결재계좌가 선택되지 않았습니다.');
                                      return;
                                    }

                                    var alert = await showAlertDl(context, title: 'NULL' ?? 'NULL');
                                    if(alert == false) {
                                      WidgetT.showSnackBar(context, text: '시스템에 저장을 취소했습니다.');
                                      return;
                                    }

                                    for(var revenue in revenueMaps.values) {
                                      revenue.ctUid = ct.id;
                                      revenue.csUid = ct.csUid;
                                      await FireStoreT.updateRevenue(revenue);
                                      await FireStoreT.updateItemTS(ItemTS.fromRe(revenue), DateTime.fromMicrosecondsSinceEpoch(revenue.revenueAt));
                                      if(payment == '즉시') await FireStoreT.updateTransaction(TS.fromRe(revenue, payType));
                                    }
                                  ...
                                  child: Row( mainAxisSize: MainAxisSize.min,
                                    children: [
                                      WidgetT.iconMini(Icons.save),
                                      Text('매출추가', style: StyleT.titleStyle(),),
                                      SizedBox(width: 12,),
                                    ],
                                  )
                              ),
