...
class DatabaseM {
  ...
  static dynamic updateEmployee(Employee data, { Map<String, Uint8List>? files, }) async {
    ...
    try {
      ...
      await FireStoreHub.docUpdate('employee/${data.id}', 'EM.PATCH', data.toJson(),);
    } catch (e) {
      if(files != null) {
        for(int i = 0; i < files.length; i++) {
          var f = files.values.elementAt(i);
  ...
  static dynamic getReWithDate(int start, int end) async {
    ...
    await coll.where('revenueAt', isGreaterThan: start).where('revenueAt', isLessThan: end).orderBy('revenueAt', descending: true).limit(100).get().then((value) {
  ...
  static dynamic updatePurchase(Purchase data, { Purchase? org,  Map<String, Uint8List>? files, }) async {
    ...
    var dateId = StyleT.dateFormatM(DateTime.fromMicrosecondsSinceEpoch(data.purchaseAt));
    ...
    if(files != null) {
      try {
        for(int i = 0; i < files.length; i++) {
          ...
          final mountainImagesRef = FirebaseStorage.instance.ref().child("purchase/${data.id}/${k}");
    ...
    /// 거래처 매입문서 읽기 효율화
    final csDetailRef = db.collection('customer/${data.csUid}/cs-dateH-purchase').doc(dateIdHarp);
    final orgCsDetailRef = db.collection('customer/${data.csUid}/cs-dateH-purchase').doc(orgDateIdHarp);
    /// 검색 기록 문서 경로로
   final searchRef = db.collection('meta/search/dateQ-purchase').doc(dateIdQuarter);
    ...
    db.runTransaction((transaction) async {
      ...
      if(searchRefSn.exists) {
        transaction.update(searchRef, { 'list\.${data.id}': searchText, 'updateAt': DateTime.now().microsecondsSinceEpoch, },);
      } else {
  ...
  static dynamic deletePu(Purchase data,) async {
    ...
    var csPurRef = db.collection('customer/${data.csUid}/cs-dateH-purchase').doc(dateIdHarp);
    var csPurSn = await csPurRef.get();
    if(csPurSn.exists) batch.update(csPurRef, {'list\.${data.id}': null, 'updateAt': DateTime.now().microsecondsSinceEpoch,});

    var csRef = db.collection('customer').doc(data.csUid);
    batch.update(csRef, {'puCount': FieldValue.increment(-1), 'updateAt': DateTime.now().microsecondsSinceEpoch,});

    var searchRef = db.collection('meta/search/dateQ-purchase').doc(dateIdQuarter);
    var searchSn = await searchRef.get();
    if(searchSn.exists) batch.update(searchRef, {'list\.${data.id}': null, 'updateAt': DateTime.now().microsecondsSinceEpoch,});
  ...
  static dynamic getPurchaseWithDate(int start, int end) async {
    ...
    await coll.where('purchaseAt', isGreaterThan: start).where('purchaseAt', isLessThan: end).orderBy('purchaseAt', descending: true)
  ...
  static dynamic initStreamRevenue() async {
    CollectionReference coll = await FirebaseFirestore.instance.collection('revenue');
    await coll.orderBy('revenueAt', descending: true).limit(50).snapshots().listen((value) {
      print('get revenue data');
      if(value.docs == null) return;
      print(value.docs.length);
  ...
  static dynamic getRevenueOnlyContract(String ctUid) async {
    List<Revenue> data = [];
    CollectionReference coll = await FirebaseFirestore.instance.collection('revenue');
    await coll.orderBy('state').orderBy('revenueAt', descending:  true).where('state', isNotEqualTo: 'DEL')
  ...
  static dynamic getRevenueWithCS(String id) async {
    List<Revenue> data = [];
    CollectionReference coll = await FirebaseFirestore.instance.collection('revenue');
    await coll.where('csUid', isEqualTo: id).where('state', whereIn: [ '' ]).get().then((value) {
      print('get revenue data');
      if(value.docs == null) return false;
      print(value.docs.length);
  ...
  static dynamic getVersionInfo() async {
    ...
    CollectionReference coll = await FirebaseFirestore.instance.collection('meta/version/log');
    await coll.orderBy('updateAt', descending: true).limit(25).get().then((value) {
      if(value.docs == null) return false;
      print(value.docs.length);
  ...
  static dynamic updateAccount(Account ac) async {
    ...
    DocumentReference doc = await FirebaseFirestore.instance.doc('meta/account');
    try {
      await doc.update({
      ...
      await FireStoreError.setDocument('meta/account', 'AC.PATCH', { 'list' : { ac.id: ac.toJson() }});
    }
  }
  ...
  static dynamic updateTransactionDate(TS ts) async {
    ...
    CollectionReference coll = await FirebaseFirestore.instance.collection('meta/date-m/transaction');
    try {
  ...
  static dynamic getTransaction({ String? startAt, int? startDate, int? lastDate, }) async {
    List<TS> tsList = [];
    if(startAt != null) {
      var doc = await FirebaseFirestore.instance.doc('transaction/$startAt').get();
      ...
      if(startDate != null && lastDate != null) {
        await FirebaseFirestore.instance.collection('transaction')
            ...
            .where('transactionAt', isLessThan: lastDate).where('transactionAt', isGreaterThan: startDate)
        ...
        await FirebaseFirestore.instance.collection('transaction').orderBy('transactionAt', descending: true)
            .where('type', whereIn: [ 'RE', 'PU' ])
            .startAfterDocument(doc).limit(25).get().then((value) {
          print('get transaction data');
          if(value.docs == null) return;
  ...
  static dynamic getTransactionCt(String id) async {
    ...
    await coll.orderBy('transactionAt', descending: true).where('ctUid', isEqualTo: id).get().then((value) {
      print('get transaction data');
      if(value.docs == null) return false;
      print(value.docs.length);
  ...
  static dynamic updateTsAtCs(TS data, bool isCreate) async {
    CollectionReference coll = await FirebaseFirestore.instance.collection('customer/${data.csUid}/detail');
    try {
    ...
    try {
      if (isCreate) await coll2.doc(data.csUid).update({'tsCount': FieldValue.increment(1),});
    } catch (e) {
      print(e);
    }
  }
  ...
  static dynamic updateTsMetaMeta(String id) async {
    CollectionReference coll = await FirebaseFirestore.instance.collection('meta/search/transaction-meta');
    try {
      await coll.doc(id).update({'count': FieldValue.increment(1),});
    } catch (e) {
      await coll.doc(id).set({'count': FieldValue.increment(1),});
    }
    CollectionReference coll2 = await FirebaseFirestore.instance.collection('meta');
    try {
  ...
  static dynamic getTransactionQr_CsReDt(String id) async {
    Map<dynamic, TS> datas = {};
    CollectionReference coll = await FirebaseFirestore.instance.collection('transaction');
    await coll.where('csUid', isEqualTo: id).where('type', isEqualTo: 'RE').get().then((value) {
      print('get transaction data');
      if(value.docs == null) return false;
      print(value.docs.length);
  ...
  static dynamic getTsWithCsDoc(String id) async {
    Map<dynamic, TS> datas = {};
    CollectionReference coll = await FirebaseFirestore.instance.collection('customer/${id}/detail');
  ...
  static dynamic initStreamPurchase() async {
    CollectionReference coll = await FirebaseFirestore.instance.collection('purchase');
    await coll.orderBy('purchaseAt', descending: true).limit(50).snapshots().listen((value) {
      print('get purchase data');
      if(value.docs == null) return;
      print(value.docs.length);
  ...
  static dynamic getPur_withCS(String id) async {
    ...
    await coll.orderBy('purchaseAt', descending: true)
        .where('csUid', isEqualTo: id).where('state', whereIn: [ '' ]).get().then((value) {
      print('get purchase data');
      if(value.docs == null) return false;
      print(value.docs.length);
  ...
  static dynamic getPurchaseDelete({ String? startAt }) async {
    ...
    if(startAt != null) {
      ...
      await coll.orderBy('purchaseAt', descending: true)
          .where('state', isEqualTo: 'DEL').startAfterDocument(doc).limit(10).get().then((value) {
        if(value.docs == null) return false;
        print(value.docs.length);
  ...
  static dynamic getPurchase({ String? startAt, int? startDate, int? lastDate, }) async {
    List<Purchase> data = [];
    ...
    if(startAt != null) {
      ...
      if(startDate != null && lastDate != null) {
        await coll.orderBy('purchaseAt', descending: true)
            .where('purchaseAt', isLessThanOrEqualTo: lastDate).where('purchaseAt', isGreaterThanOrEqualTo: startDate)
            .where('state', whereIn: [ '' ],)
            .startAfterDocument(doc).limit(25).get().then((value) {
          if(value.docs == null) return false;
          print(value.docs.length);
  ...
  static dynamic getPurchaseAll({ String? startAt, int? startDate, int? lastDate, }) async {
    List<Purchase> data = [];
    CollectionReference coll = FirebaseFirestore.instance.collection('purchase');
    await coll.orderBy('purchaseAt', descending: true)
  ...
  static dynamic getRevenue({ String? startAt, int? startDate, int? lastDate,}) async {
    List<Revenue> data = [];
    CollectionReference coll = FirebaseFirestore.instance.collection('revenue');
    ...
    if(startAt != null) {
      ...
      if(startDate != null && lastDate != null) {
        await coll.orderBy('revenueAt', descending: true)
            .where('revenueAt', isLessThan: lastDate).where('revenueAt', isGreaterThan: startDate)
            .startAfterDocument(doc).limit(25).get().then((value) {
          if(value.docs == null) return false;
          print(value.docs.length);
        ...
        await coll.orderBy('state').orderBy('revenueAt', descending: true)
            .where('state', isNotEqualTo: 'DEL').startAfterDocument(doc).limit(25).get().then((value) {
          if(value.docs == null) return false;
          print(value.docs.length);
  ...
  static dynamic updateContract(Contract data, { Map<String, Uint8List>? files, Map<String, Uint8List>? ctFiles, }) async {
    if(data.id == '') data.id = generateRandomString(16);

    /// 메타값 무결성 확인
    await getCtMetaCountCheck();
    if(data.scUid == '') data.scUid = SystemT.searchMeta.contractCursor.toString();
    if(data.csName == '') data.csName = (await SystemT.getCS(data.csUid)).businessName;
    ...
    try {
      ...
      await FireStoreHub.docUpdate('contract/${data.id}', 'CT.PATCH', data.toJson(),);
      await FireStoreHub.docUpdate('meta/search/contract/${data.scUid}', 'CT.META.PATCH',
          { 'list\.' + data.id:  data.csName + '&:' + data.ctName + '&:' + data.fileName + '&:' + data.manager + '&:' + data.memo, },
          setJson: { 'list': { data.id:  data.csName + '&:' + data.ctName + '&:' + data.fileName + '&:' + data.manager + '&:' + data.memo, } },);
    } catch (e) {
      if(files != null) {
        for(int i = 0; i < files.length; i++) {
          var f = files.values.elementAt(i);
  ...
  static dynamic deleteContract(Contract data,) async {
    data.state = 'DEL';
    await FireStoreHub.docUpdate('contract/${data.id}', 'CT.DELETE', data.toJson(),);
    await FireStoreHub.docUpdate('meta/search/contract/${data.scUid}', 'CT-Meta.DELETE',
      { 'list\.' + data.id:  '', },
      setJson: { 'list': { '', } },);
  }
  static dynamic initStreamContract() async {
    /*.where('state', isNotEqualTo: 'DEL', isNull: true)*/
    await FirebaseFirestore.instance.collection('contract').limit(50).snapshots().listen((value) async {
  ...
  static dynamic getCtMetaCountCheck() async {
    var aaa = 0;
    CollectionReference coll = await FirebaseFirestore.instance.collection('meta/search/contract-meta');
    await coll.doc(SystemT.searchMeta.contractCursor.toString()).get().then((value) {
  ...
  static dynamic initStreamCtMeta() async {
    ...
    CollectionReference coll = await FirebaseFirestore.instance.collection('meta/search/contract');
    await coll.snapshots().listen((value) {
      ...
      for(var a in value.docs) {
        ...
        FireStoreHub.docUpdate('meta/search/contract-meta/${a.id}', 'CT.META-COUNT.PATCH',
  ...
  static dynamic initStreamPuMeta() async {
    CollectionReference coll = await FirebaseFirestore.instance.collection('meta/search/dateQ-purchase');
    await coll.snapshots().listen((value) {
      if(value.docs == null) return;
      print(value.docs.length);
  ...
  static dynamic updateCustomer(Customer data, { Map<String, Uint8List>? files }) async {
    if(data.id == '') data.id = generateRandomString(16);
  ...
  static dynamic initStreamCSMeta() async {
    ...
    await coll.doc('customer').snapshots().listen((value) {
      ...
      SystemT.customerSearch.clear(); search.clear();
      var sMap = data['search'] as Map;
      sMap.forEach((key, value) { if(value != 'DEL') search[key] = value; });
  ...
  static dynamic getCustomerDelete({String? startAt}) async {
    ...
    if(startAt != null)  {
      var startAtDoc = await FirebaseFirestore.instance.doc('customer/$startAt').get();
      query = FirebaseFirestore.instance.collection('customer').where('state', isEqualTo: 'DEL').startAfterDocument(startAtDoc).limit(10);
    }
    else {
      query = FirebaseFirestore.instance.collection('customer').where('state', isEqualTo: 'DEL').limit(10);
    }
  ...
  static dynamic getContract({String? startAt}) async {
    ...
    if(startAt != null) {
      ...
      await FirebaseFirestore.instance.collection('contract').orderBy('state')
          .where('state', isNotEqualTo: 'DEL').startAfterDocument(doc).limit(50).get().then((value) {
        if(value.docs == null) return false;
        print(value.docs.length);
  ...
  static dynamic getFactory({ String? startAt, int? startDate, int? lastDate, }) async {
    List<FactoryD> factory_list = [];
    if(startAt != null) {
      if(startAt == '') return factory_list;

      var doc = await FirebaseFirestore.instance.doc('factory-daily/$startAt').get();

      if(startDate != null && lastDate != null) {
        await FirebaseFirestore.instance.collection('factory-daily')
            .orderBy('date', descending: true)
            .where('date', isLessThan: lastDate).where('date', isGreaterThan: startDate)
            .where('state', whereIn: [ '' ])
            .startAfterDocument(doc).limit(25).get().then((value) {
          if(value.docs == null) return;
          print(value.docs.length);
        ...
        await FirebaseFirestore.instance.collection('factory-daily').orderBy('date', descending: true)
            .where('state', whereIn: [ '' ])
            .startAfterDocument(doc).limit(25).get().then((value) {
          print('get transaction data');
          if(value.docs == null) return;
  ...
  static dynamic getProductList({ String? startAt, int? startDate, int? lastDate, }) async {
    List<ProductD> factory_list = [];
    if(startAt != null) {
      var doc = await FirebaseFirestore.instance.doc('product-daily/$startAt').get();
      ...
      if(startDate != null && lastDate != null) {
        ...
        await FirebaseFirestore.instance.collection('product-daily').orderBy('date', descending: true)
            .where('state', whereIn: [ '' ])
            .startAfterDocument(doc).limit(25).get().then((value) {
          print('get transaction data');
          if(value.docs == null) return;
    ...
    try {
      if(files != null) {
        for(int i = 0; i < files.length; i++) {
          ...
          var url = await StorageHub.updateFile('product-daily/${data.id}', 'PD-Date', f, k);
      ...
      await FireStoreHub.docUpdate('product-daily/${data.id}', 'PD-Date.PATCH', data.toJson(),);
    } catch (e) {
      print(e);
      if(files != null) {
        for(int i = 0; i < files.length; i++) {
  ...
  static dynamic initStreamProductDate() async {
    CollectionReference coll = await FirebaseFirestore.instance.collection('product-daily');
    await coll.orderBy('date', descending: true).limit(50).snapshots().listen((value) {
  ...
  static dynamic getSchedule() async {
    ...
    await coll.orderBy('date', descending: true).limit(50).get().then((value) {
  ...
  static dynamic getSCH_CT(String id) async {
    List<Schedule> data = [];
    CollectionReference coll = await FirebaseFirestore.instance.collection('schedule');
    await coll.orderBy('date', descending: true).where('ctUid', isEqualTo: id).limit(50).get().then((value) {
      print('get schedule data');
      if(value.docs == null) return false;
      print(value.docs.length);
  ...
  static dynamic updateItemTS(ItemTS data, DateTime date) async {
    ...
    CollectionReference coll = await FirebaseFirestore.instance.collection('inventory');
    try {
  ...
  static dynamic updateItemMetaGP(String id, String group, { Map<dynamic, dynamic>? groupAll }) async {
    CollectionReference coll = await FirebaseFirestore.instance.collection('meta');
